...
app.get('/api/customers/:id/result/shopping', async (req, res) => {
  ...
  try {
    const query = knex('shopping_list').select('shopping_list.id', 'shopping_list.storeId', 'shopping_list.productName', 'shopping_list.piece', 'shopping_list.unit', 'shopping_list.flag', 'store_list.storeName').join('store_list', 'shopping_list.storeId', '=', 'store_list.id').where('shopping_list.userId', userId);
